...
namespace Etechflow\MetaTemplates\Block\Adminhtml\System\Config;
...
/**
 * Status banner at the top of the Meta Templates admin config section.
 *
...
class ModuleStatus extends Fieldset
{
    ...
    private function renderStatusBanner(): string
    {
        ...
        if (!$licenceValid) {
            if (!$hasKey) {
                return $this->banner(
                    ...
                    . 'The module is silently disabled — meta-template rules do not apply on the storefront, and the '
                    . 'admin Templates page redirects to the licence gate. Paste your key in the <strong>License Key</strong> '
                    . 'field below, or visit '
                    . '<a href="' . $this->getUrl('metatemplates/license/gate') . '" style="color:inherit;text-decoration:underline;">License &amp; Plans</a> '
                    . 'to purchase one.'
                );
            }
        ...
        return $this->banner(
            'success',
            '&#9989; Module is active',
            'Licence valid for <code>' . $this->escapeHtml($host) . '</code>. Your meta-template rules are applied to '
            . 'product, category, and CMS meta titles/descriptions/keywords.'
        );
    }
